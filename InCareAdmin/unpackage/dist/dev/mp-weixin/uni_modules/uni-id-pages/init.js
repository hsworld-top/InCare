...
async function uniIdPageInit() {
  if (debug) {
    ...
    common_vendor.index.__f__("log", "at uni_modules/uni-id-pages/init.js:40", loginTypes);
    common_vendor.index.__f__("log", "at uni_modules/uni-id-pages/init.js:41", supportedLoginType);
    const list = loginTypes.filter((type) => !supportedLoginType.includes(data[type]));
    common_vendor.index.__f__("log", "at uni_modules/uni-id-pages/init.js:43", list);
    if (list.length) {
      common_vendor.index.__f__(
        "error",
        "at uni_modules/uni-id-pages/init.js:45",
